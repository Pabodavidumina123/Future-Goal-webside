...
const CourseCard = ({ course }) => {
  ...
  return (
    <Link
      to={`/courses/${_id}`}
      className="glass-card flex flex-col h-full rounded-2xl overflow-hidden shadow-lg hover:shadow-indigo-500/10 group cursor-pointer"
    >
      ...
      <div className="p-6 flex flex-col flex-grow justify-between gap-4">
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          <span className="text-xs font-bold text-indigo-400 group-hover:text-indigo-300 flex items-center gap-1 transition-all group-hover:translate-x-1">
            View Details &rarr;
          </span>
        </div>
      </div>
